...
const ShareProductModal: React.FC<ShareProductModalProps> = ({
  ...
  const shareText = `Check out ${product.title} - authentic handcrafted product from CraftBizz Marketplace!`;
  ...
  const shareLinks = [
    ...
    },
    {
      ...
      url: `https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(productUrl)}&title=${encodeURIComponent(product.title)}`,
          ...
          <div className="grid grid-cols-4 gap-2">
            {shareLinks.map((link) => (
              <a
                ...
                className={`flex flex-col items-center justify-center gap-1 p-3 rounded-lg text-white transition-colors ${link.color}`}
              >
                {link.icon}
